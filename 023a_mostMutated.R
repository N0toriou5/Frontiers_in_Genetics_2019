...
matrix2col<-function(z,col1="white",col2="black",nbreaks=1000,center=FALSE,minmax=NULL){
    if(is.null(minmax)){
        if(center){
...
### Now we investigate the most mutated genes.
...
# Most Mutated Genes in Individual Cancers
seengenes<-c()
for(tum in names(tums)){
    ...
    png(paste0("plots/023a_mostMutated/mutations_",tum,".png"),w=3000,h=2000,p=40)
    par(las=2)
    bp<-barplot(100*x/ncol(mutations),ylab="% Mutated Samples",
                col=ifelse(names(x)%in%eg2sym(mutsig),"grey","black"),
...
length(seengenes) # 20408 genes with at least one somatic mutations in at least one cancer (basically most of the genome)
if(TRUE){seengenes<-setdiff(seengenes,mutsig)} # Ignoring MutSig blacklisted genes
length(seengenes) # 20303 genes
...
dim(panmat) # 6626 genes with at least 10 mutations in at least one tumor type, 33 tumor types
...
for(tum in names(tums)){
    ...
    hypermut[tum]<-sum(sumhere>=length(seengenes)*0.005)/ncol(mutations) # "Hypermutated" samples with more than 0.5% of the genes somatically mutated
    specialists[tum]<-names(freqhere)[which.max(freqhere)]
}
pancancer_frequency<-apply(panmat,1,function(x){sum(x)/length(x)})

### Genes to show
topgenes<-names(sort(pancancer_frequency,dec=TRUE))[1:25]
...
###### Complicated layout ######
png("plots/023a_mostMutated.png",w=4000,h=4000,p=100)
lmatrix<-rbind(c(1,1,2),c(3,4,5),c(6,7,8))
layout(lmatrix,width=c(1,6,1),heights=c(1,6,2))

# 1- TITLE
par(mar = c(0,0,0,0))
texttitle<-"Frequent Somatic Mutations in Pan-Cancer Dataset"
...
radius<-0.35
circlecols<-matrix2col(central,minmax=c(0,1))$colormatrix
...
# 5- RIGHT BARPLOT
par(mar = c(0,0,0,0.4))
bp<-barplot(rev(rightbarplot),horiz=TRUE,add=TRUE,col="black",offset=0,axes=FALSE,plot=FALSE)
plot(1,xlim=c(0,0.4),ylim=c(min(bp),max(bp)),ann=F,type="n",axes=FALSE)
barplot(rev(rightbarplot),horiz=TRUE,add=TRUE,col="black",offset=0,axes=FALSE,names.arg=NA)
...
text(0.5,-0.4,labels="HyperMutated\nSamples\n(%)",xpd=NA)
...
text(2.5,12,"Mutated\nSamples")
ii<-0
icols<-colorpanel(6,"white","black")[2:6]
