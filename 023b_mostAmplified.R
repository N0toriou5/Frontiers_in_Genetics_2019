...
matrix2col<-function(z,col1="white",col2="red3",nbreaks=1000,center=FALSE,minmax=NULL){
    if(is.null(minmax)){
        if(center){
...
### Now we investigate the most amplified genes.
...
# Most amplified Genes in Individual Cancers
seengenes<-c()
for(tum in names(tums)){
    ...
    if(nrow(fcnv)>1){
        ...
        png(paste0("plots/023b_mostAmplified/amplifications_",tum,".png"),w=3000,h=2000,p=40)
        par(las=2)
        bp<-barplot(100*x/ncol(mutations),ylab="% Amplified Samples",
                    col=ifelse(names(x)%in%eg2sym(mutsig),"grey","red3"),
...
for(tum in names(tums)){
    ...
    if(nrow(fcnv)>1){
        ...
        hypermut[tum]<-sum(sumhere>=length(seengenes)*0.005)/ncol(mutations) # "Hyperamplified" samples with more than 0.5% of the genes somatically amplified
        specialists[tum]<-names(freqhere)[which.max(freqhere)]
    }
}
pancancer_frequency<-apply(panmat,1,function(x){sum(x)/length(x)})
picked<-sym2eg(c("ERBB2","EGFR","MDM2","CCND1"))
...
###### Complicated layout ######
png("plots/023b_mostAmplified.png",w=4000,h=4000,p=100)
lmatrix<-rbind(c(1,1,2),c(3,4,5),c(6,7,8))
layout(lmatrix,width=c(1,6,1),heights=c(1,6,2))

# 1- TITLE
par(mar = c(0,0,0,0))
texttitle<-"Frequent Amplifications in Pan-Cancer Dataset"
...
barplot(rev(rightbarplot),horiz=TRUE,add=TRUE,col="red3",offset=0,axes=FALSE,names.arg=NA)
...
text(0.5,-0.4,labels="HyperAmplified\nSamples\n(%)",xpd=NA)
...
text(2.5,12,"Amplified\nSamples")
ii<-0
icols<-colorpanel(6,"white","red3")[2:6]
