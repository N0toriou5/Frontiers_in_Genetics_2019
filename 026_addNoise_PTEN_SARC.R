...
fname<-paste0("results/026_",tum,"_",eg2sym(gene),"_del_aurocs0.rda")
...
for(noise in rev(noises)){
    for(inp in 1:nperm){
        fname<-paste0("results/026_addNoise/",gene,"_",tum,"_noise",noise,"_aurocs_",inp,".rda")
        if(!file.exists(fname)){
            aurocs<-c()
...
# Plot
png(paste0("plots/026_noise_",tum,"_del_",eg2sym(gene),".png"),w=3000,h=2000,p=60)
par(mar=c(4,4,1,0))
results<-results_exp
means<-c(aurocs0[1],apply(results,2,mean,na.rm=TRUE));names(means)[1]<-"0"
sds<-c(0,apply(results,2,sd,na.rm=TRUE));names(means)[1]<-"0"
plotCI(means,uiw=sds,liw=0,pch=16,type="o",lwd=6,col="black",cex=1.5,xaxt="n",ylab="AUROC",xlab="Gaussian Noise Level",ylim=c(0.4,0.8),gap=0,
       main="PTEN deletions - classification model in Sarcoma",add=FALSE)
