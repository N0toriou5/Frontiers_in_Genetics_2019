...
################ EGFR amplifications in GBM #################
tum<-"GBM"
gene<-any2entrez("EGFR")
...
fname<-paste0("results/026_",tum,"_",eg2sym(gene),"_amp_aurocs0.rda")
...
# Plot
png(paste0("plots/026_noise_",tum,"_amp_",eg2sym(gene),".png"),w=3000,h=2000,p=60)
par(mar=c(4,4,1,0))
results<-results_exp
...
plotCI(means,uiw=sds,liw=0,pch=16,type="o",lwd=6,col="black",cex=1.5,xaxt="n",ylab="AUROC",xlab="Gaussian Noise Level",ylim=c(0.4,1.0),gap=0,
       main="EGFR amplifications - classification model in Glioblastoma",add=FALSE)
